...
function setEm(e) {
    ...
    $('#chat-content').html(function () {
        return $(this).html() + emstr;
    }).focus();
}
...
// 优选网络
    $('#btn-network').bind('click',function(){
        ...
        if($(".video-mask").is(":hidden")){
            ...
            var l = JSON.parse(document.getElementById('livePlayer').getLine());
            var netlist = '';
    ...
    $('#btn-netsubmit').bind('click',function(){
        ...
        var l = document.getElementById('livePlayer');

        $('#network').fadeOut(200);

        if($(".video-mask").is(":hidden")){
            l.changeLine(network);
        }
